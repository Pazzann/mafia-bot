...
export const curHandlingGames: Map<number, IMafiaGameProps> = new Map();
...
discordBot.on('interactionCreate', async (interaction: Interaction) => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        let game: IMafiaGameProps | null = null;
        let user: IUserProps | null = null;

        for (let v of curHandlingGames.values()) {
            if (v.users.filter((item: IUserProps) => item.userid === interaction.user.id).length > 0) {
                game = v;
                user = v.users.filter((item: IUserProps) => item.userid === interaction.user.id)[0];
            }
        }
        if (user || game) {
            if (user.isKilled) {
