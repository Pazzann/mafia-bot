...
export default async function deleteroleselect(interaction: SelectMenuInteraction, user: User, locale: ILangProps, roleId: number) {
    ...
    try {
        ...
        if (role == null) {
            interaction.reply({content: locale.role_delete_error_notFound, ephemeral: true}).catch();
            return;
        }
        if (role.user.userid != user.userid) {
            interaction.reply({content: locale.role_delete_error_noAccess, ephemeral: true}).catch();
            return;
        }
        await Role.delete({id: roleId});
        interaction.reply({content: locale.role_delete_success_message, ephemeral: true}).catch();
    } catch (err) {
